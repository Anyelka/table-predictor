...
const buttonVariants = {
  default: {
    opacity: 1,
    x: 0,
    background: "linear-gradient(90deg, #00db74, #00c0cf)",
  },
  selected: {
    opacity: 1,
    x: 0,
    background: "linear-gradient(90deg, #37003c, #7f3985)",
    color: "#00ff88",
  },
  predictionActive: {
    opacity: 1,
  ...
  underway: {
    opacity: 1,
    x: 0,
    background: "linear-gradient(90deg, #00ff88, #2dbdff)",
  },
  hover: { scale: 1.1, x: 0 },
...
const getVariant = (isSelected, isPredictionActive, isUnderway, disabled) => {
  if (isSelected) {
    return "selected";
  }
